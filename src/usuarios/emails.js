...
class EmailRedefinicaoSenha extends Email {
  constructor(usuario, token) {
    ...
    this.text = `Olá! Você solicitou uma redefinição de senha. Aqui está seu token de redefinição de senha ${token}`
    this.html = `<h1>Olá!</h1> Você solicitou uma redefinição de senha. Aqui está seu token de redefinição de senha:
    ${token}`
  }
}
